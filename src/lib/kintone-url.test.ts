...
describe('parseKintoneAppUrl', () => {
  ...
  it('空文字列でエラーを投げる', () => {
    expect(() => parseKintoneAppUrl('')).toThrow('アプリURLが入力されていません');
  });

  it('無効なURLでエラーを投げる', () => {
    expect(() => parseKintoneAppUrl('invalid-url')).toThrow('有効なURLを入力してください');
  });

  it('HTTPでエラーを投げる', () => {
    expect(() => parseKintoneAppUrl('http://example.cybozu.com/k/123/')).toThrow(
      'HTTPSのURLを入力してください'
    );
  });
  ...
  it('アプリIDが数字でない場合エラーを投げる', () => {
    expect(() => parseKintoneAppUrl('https://example.cybozu.com/k/abc/')).toThrow(
      'kintone アプリのURLを入力してください'
    );
  });
});
...
describe('isValidKintoneAppUrl', () => {
  it('有効なURLでtrueを返す', () => {
    expect(isValidKintoneAppUrl('https://example.cybozu.com/k/123/')).toBe(true);
  });
