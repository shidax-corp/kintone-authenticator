...
export const readQRFromImage = async (imageUrl: string): Promise<string> => {
  try {
    ...
    throw new QRReadError(`QR reading failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
};
...
export const readQRFromElement = async (element: HTMLElement): Promise<string> => {
