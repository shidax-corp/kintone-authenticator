...
describe('gen-otp', () => {
  describe('generateHOTP', () => {
    const opts: HOTPRequest = {
      secret: new Uint8Array([0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30]),  // '12345678901234567890' in ASCII
      algorithm: 'SHA-1',
      digits: 6,
    };
    ...
    it.each(tests)('HOTP at counter $counter', async ({ counter, expected }) => {
  ...
  describe('generateTOTP', () => {
    ...
    it.each(tests)('TOTP HMAC-$algorithm at time $time', async ({ time, algorithm, expected }) => {
      const opts = {
